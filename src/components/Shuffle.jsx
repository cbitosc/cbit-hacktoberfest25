...
const Shuffle = ({
  ...
  const responsiveSizeClass = responsiveSize 
    ? 'text-4xl sm:text-4xl md:text-5xl lg:text-6xl xl:text-7xl' 
    : 'text-[4rem]';

  const baseTw = `inline-block whitespace-normal break-words will-change-transform uppercase leading-none ${responsiveSizeClass}`;
  ...
  // If custom fontSize object is provided, use inline styles
  if (!responsiveSize && typeof fontSize === 'object') {
    commonStyle.fontSize = fontSize.mobile;
    
    // Add media query styles
    if (typeof window !== 'undefined') {
      const mq = window.matchMedia('(min-width: 768px)');
      if (mq.matches) commonStyle.fontSize = fontSize.tablet;
      
      const mqLg = window.matchMedia('(min-width: 1024px)');
      if (mqLg.matches) commonStyle.fontSize = fontSize.desktop;
    }
  }
