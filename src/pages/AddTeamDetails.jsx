...
import { validateTeamEmails, validateSingleTeammateEmail } from '../services/emailValidationService';
import { 
  User, 
  GraduationCap, 
  Calendar, 
...
const AddTeamDetails = ({ 
  ...
  const [techStack, setTechStack] = useState(() => {
    ...
    if (initialData && initialData.techStack) {
      ...
      } else if (typeof initialData.techStack === 'string' && initialData.techStack) {
  ...
  useEffect(() => {
    ...
    validateFormSync();
  }, [teammates, techStack, emailErrors, phoneErrors, teamSize, initialData, leaderData]);
  ...
  const revalidateAllEmails = async (teammates) => {
    ...
    try {
      const teammateEmails = teammates.map(t => t.email).filter(e => e && e.trim());
      const existingTeammateEmails = initialData && initialData.teammates 
        ? initialData.teammates.map(t => t.email).filter(e => e && e.trim())
        : [];
  ...
  const handleTeammateChange = async (index, field, value) => {
    ...
    if (field === "collegeType") {
      updatedTeammates[index] = {
        ...updatedTeammates[index],
        collegeType: value,
        college: value === "CBIT" ? "Chaitanya Bharathi Institute of Technology" : "",
        customCollege: value === "CBIT" ? "" : updatedTeammates[index].customCollege,
      };
  ...
  const validateForm = async () => {
    try {
      ...
      if (teamSize > 1) {
        const requiredFields = ['name', 'degree', 'yearOfStudy', 'branch', 'rollNumber', 'email', 'phoneNumber'];
        const allFieldsValid = teammates.every(teammate => {
          ...
          const collegeValid = 
            ...
            (teammate.collegeType === "Other" && teammate.customCollege && teammate.customCollege.trim() !== "");
        ...
        const hasEmailErrors = Object.values(emailErrors).some(error => error !== null && error !== undefined);
        if (hasEmailErrors) {
          return false;
        }
        ...
        const emailValidation = await validateTeamEmails(leaderData.email, teammateEmails, existingTeammateEmails);
        ...
        const hasPhoneErrors = Object.values(phoneErrors).some(error => error !== null && error !== undefined);
        if (hasPhoneErrors) {
          return false;
        }
      }
      ...
      if (teamSize < 3 && (!Array.isArray(techStack) || techStack.length === 0)) {
  ...
  const scrollToFirstError = () => {
    ...
    const firstEmailErrorIndex = Object.entries(emailErrors)
      .filter(([key, value]) => value !== null && value !== undefined && !isNaN(parseInt(key)))
      .map(([key]) => parseInt(key))
      .sort((a, b) => a - b)[0];
    
    if (firstEmailErrorIndex !== undefined && teammateRefs.current[firstEmailErrorIndex]) {
    ...
    if (firstPhoneErrorIndex !== undefined && teammateRefs.current[firstPhoneErrorIndex]) {
    ...
    for (let i = 0; i < teammates.length; i++) {
      ...
      const hasEmptyField = !teammate.name || !teammate.email || !teammate.phoneNumber || 
                           !teammate.degree || !teammate.yearOfStudy || !teammate.branch || 
                           !teammate.rollNumber || !teammate.collegeType ||
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="text-center">
        ...
        >
          {initialData ? 'Edit Team Details' : (teamSize === 1 ? 'Additional Information' : 'Add Teammate Details')}
        </h2>
        <p 
        ...
        >
          {initialData 
            ...
            : (teamSize === 1 
              ...
              : `Enter details for your ${teamSize - 1} teammate${teamSize > 2 ? 's' : ''}`)
          }
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        {teamSize > 1 && teammates.map((teammate, index) => (
          <div 
            key={index}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                ...
                <select
                  ...
                  onChange={(e) => handleTeammateChange(index, 'collegeType', e.target.value)}
                  required
                ...
                >
                  ...
                  <option value="CBIT">CBIT (Chaitanya Bharathi Institute of Technology)</option>
                ...
                {teammate.collegeType === "Other" && (
                  <input
                    ...
                    onChange={(e) => handleTeammateChange(index, 'customCollege', e.target.value)}
                    required
              ...
              <div className="space-y-2">
                ...
                <select
                  ...
                  onChange={(e) => handleTeammateChange(index, 'yearOfStudy', e.target.value)}
                  required
              ...
              <div className="space-y-2">
                ...
                {teammate.branchType === "Other" && (
                  <input
                    ...
                    onChange={(e) => handleTeammateChange(index, "customBranch", e.target.value)}
                    required
                    className="w-full px-4 py-3 rounded-lg border-2 bg-[#403F7D] text-white placeholder-[#D0CCE3] focus:border-[#8A86FF] focus:outline-none transition-all duration-300 mt-2"
                    style={{
                      borderColor: "#5E577C",
              ...
              <div className="space-y-2">
                ...
                {emailErrors[index] && (
                  <p className="text-red-400 text-xs mt-1" style={{ fontFamily: "'Atkinson Hyperlegible', sans-serif" }}>
                    {emailErrors[index]}
                  </p>
                )}
              </div>
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        if (e.target.checked) {
                          ...
                          setTechStack(techStack.filter(item => item !== option.value));
                        }
                      }}
                      className="w-5 h-5 text-[#8A86FF] bg-[#403F7D] border-[#5E577C] rounded focus:ring-[#8A86FF] focus:ring-2"
                    />
                    <span className={`text-sm font-medium ${
                      techStack.includes(option.value) ? 'text-white' : 'text-[#D0CCE3]'
                    }`}>
                      {option.label}
                    </span>
                  </label>
        ...
        <div className="flex flex-col sm:flex-row gap-4 sm:gap-0 justify-between pt-6">
          ...
          >
            {isSubmitting ? (
              ...
              <>
                <span>{initialData ? 'Update Team Details' : 'Complete Registration'}</span>
